...
```ts
// src/store/thunks/lots/fetchLotByIdThunk.ts
import type { AppThunk } from "@/store";
import { getLotById } from "@/api/lots/client";

type ResponseType = Promise<200 | 400 | 500>;

/**
 * Loads a lot by id through the API layer (manual thunk; do not use createAsyncThunk).
 */
export const fetchLotByIdThunk =
  (lotId: string): AppThunk<ResponseType> =>
  async (dispatch): ResponseType => {
    const result = await getLotById(lotId);
    if (!result.ok) return 400;
    dispatch(lotsLoaded(result.data));
    return 200;
  };
```
